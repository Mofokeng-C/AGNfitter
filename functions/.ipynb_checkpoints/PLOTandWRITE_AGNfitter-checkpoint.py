...
import math 
import numpy as np
from . import triangle #Author: Dan Foreman-Mackey (danfm@nyu.edu)
import time
import scipy
...
class OUTPUT:
    ...
    def write_parameters_outputvalues(self, P):        
        ...
        if self.out['calc_intlum']:            
            ...
            outputvalues = np.column_stack((np.transpose([(v[0],v[1],v[2],v[3],v[4]) for v in zip(*np.percentile(chain_pars, [2.5,16, 50, 84,97.5], axis=0))]),
                                            np.transpose([(v[0],v[1],v[2],v[3],v[4]) for v in zip(*np.percentile(chain_others, [2.5,16, 50,84,97.5], axis=0))]),
