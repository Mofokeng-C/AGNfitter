...
#    pickle.dump(np.array([irlum, wave, SED]), f2, protocol=2)
...
class MODEL:
    ...
    def build(self):
        ...
        if self.component== 'galaxy':
            ...
            self.wave,self.tau, self.tg,self.SED, self.SFR = pickle.load(file(self.path + 'models/GALAXY/bc03_275templates_files.pickle', 'rb'))
...
def construct(path):
    ...
    pickle.dump(TO, f1, protocol=2)
    f1.close()

    BB = MODEL('bbb', path)
    BB.build()
    f2 = open(path + 'models/BBB/richards.pickle', 'wb')
    pickle.dump(BB, f2, protocol=2)
    f2.close()
